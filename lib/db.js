...
// ── Studio users table (UI-created users, supplements env-var users) ───────────────────
db.exec(`
...
// Seed/migrate default XO theme with correct colors
(function seedDefaultTheme() {
  ...
  if (count.c === 0) {
    db.prepare(`
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?)
    `).run('xo-default', 'XO', '#FF5800', '#0c1a28', '#142233', XO_LOGO_BASE64, 1);
  } else {
    // Ensure XO default has correct colors (migration for existing installs)
    db.prepare(`
      UPDATE themes SET accent_color = '#FF5800', bg_color = '#0c1a28', header_color = '#142233', logo_base64 = ?
      WHERE id = 'xo-default' AND (accent_color != '#FF5800' OR bg_color != '#0c1a28')
    `).run(XO_LOGO_BASE64);
  }
})();
...
module.exports = {
  ...
  updateTheme(id, { name, accent_color, bg_color, header_color, logo_base64 }) {
    const fields = ['name = ?', 'accent_color = ?', 'bg_color = ?', 'header_color = ?'];
    const values = [name, accent_color, bg_color, header_color];
    if (logo_base64 !== undefined) { fields.push('logo_base64 = ?'); values.push(logo_base64); }
    values.push(id);
    return db.prepare(`UPDATE themes SET ${fields.join(', ')} WHERE id = ?`).run(...values);
  },
  ...
  // ── User settings (telegram etc.) ─────────────────────────────────────────
  getUserSetting(email) {
    return db.prepare('SELECT * FROM user_settings WHERE email = ?').get(email);
  },

  getAllUserSettings() {
    return db.prepare('SELECT * FROM user_settings').all();
  },

  setUserTelegram(email, telegram_chat_id) {
    db.prepare(`
      INSERT INTO user_settings (email, telegram_chat_id, updated_at)
      VALUES (?, ?, CURRENT_TIMESTAMP)
      ON CONFLICT(email) DO UPDATE SET telegram_chat_id = excluded.telegram_chat_id, updated_at = CURRENT_TIMESTAMP
    `).run(email, telegram_chat_id || null);
  },

  // ── Studio users (DB-managed) ─────────────────────────────────────────────────────
  getDbUsers() {
    return db.prepare('SELECT email, name, hashed_password FROM studio_users ORDER BY created_at').all();
  },

  createDbUser(email, name, hashedPassword) {
    db.prepare('INSERT INTO studio_users (email, name, hashed_password) VALUES (?, ?, ?)')
      .run(email, name, hashedPassword);
  },

  deleteDbUser(email) {
    db.prepare('DELETE FROM studio_users WHERE email = ?').run(email);
  },

  dbUserExists(email) {
    return !!db.prepare('SELECT 1 FROM studio_users WHERE email = ?').get(email);
  },

  updateDbUser(email, fields) {
    const allowed = ['name', 'hashed_password'];
    const sets = Object.keys(fields).filter(k => allowed.includes(k));
    if (!sets.length) return;
    const sql = 'UPDATE studio_users SET ' + sets.map(k => k + ' = ?').join(', ') + ' WHERE email = ?';
    db.prepare(sql).run(...sets.map(k => fields[k]), email);
  },

  deleteUserSetting(email) {
    db.prepare('DELETE FROM user_settings WHERE email = ?').run(email);
  },

  upsertUserSetting(email, key, value) {
    if (key === 'telegram_chat_id') {
      db.prepare(`INSERT INTO user_settings (email, telegram_chat_id, updated_at)
        VALUES (?, ?, datetime('now'))
