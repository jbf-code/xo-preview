...
module.exports = {
  ...
  // ── Hosted Campaigns ──────────────────────────────────────────────────────
  getAllHosted() {
    return db.prepare('SELECT * FROM hosted_campaigns ORDER BY created_at DESC').all();
  },
  ...
  addHostedFormat({ campaign_id, format_name, width, height, r2_prefix, cdn_url, tag_html, size_bytes, file_count }) {
    return db.prepare(`
      INSERT INTO hosted_formats (campaign_id, format_name, width, height, r2_prefix, cdn_url, tag_html, size_bytes, file_count)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(campaign_id, format_name, width, height, r2_prefix, cdn_url, tag_html, size_bytes || 0, file_count || 0);
  },
