  ...
  try {
    ...
    const iframeInfo = await page.evaluate(() => {
      return Array.from(document.querySelectorAll('iframe')).map((el, i) => {
        ...
        // Find format name + dimension label from Zuuvi's DOM structure
        // Structure: div[data-preview-format-id] > div > div > span.font-bold("1:1") + span("1200x1200")
        let formatName = null;
        let dimLabel = null;
        
        // Walk up to find the data-preview-format-id container
        let ancestor = el;
        for (let j = 0; j < 8 && ancestor; j++) {
          if (ancestor.dataset && ancestor.dataset.previewFormatId) {
            // Found Zuuvi format container — get the label spans
            const spans = ancestor.querySelectorAll('span');
            for (const span of spans) {
              const t = span.innerText?.trim();
              if (!t || t.length > 30 || span.children.length > 0) continue;
              // Bold span = format name ("1:1", "Story", "9:16")
              const isBold = span.classList.toString().includes('font-bold');
              if (isBold && !formatName && !/^\d{2,4}x\d{2,4}$/.test(t) && !/^\(/.test(t)) {
                formatName = t;
              }
              // Dimension span ("1200x1200")
              if (!dimLabel && /^\d{2,4}x\d{2,4}$/.test(t)) {
                dimLabel = t;
              }
            }
            break;
        ...
        if (!dimLabel) {
          ...
          for (let j = 0; j < 5 && ancestor; j++) {
            const allText = ancestor.innerText || '';
            const dimMatch = allText.match(/\b(\d{2,4})\s*[x×]\s*(\d{2,4})\b/i);
            if (dimMatch) { dimLabel = `${dimMatch[1]}x${dimMatch[2]}`; break; }
            ancestor = ancestor.parentElement;
          }
        }
        ...
        // Build label: "1:1 (1200x1200)" or just "1200x1200"
        let label = null;
        ...
        // Get original dimensions from inline style (Zuuvi uses transform:scale so style has originals)
        const styleW = parseInt((el.style.width || '').match(/(\d+)/)?.[1]) || 0;
        const styleH = parseInt((el.style.height || '').match(/(\d+)/)?.[1]) || 0;
    ...
    for (const info of bannerIframes) {
      ...
      // Parse dimensions — prefer style (original) over rendered (scaled) or attr
      let origWidth = info.styleWidth || info.attrWidth || info.renderedWidth;
      let origHeight = info.styleHeight || info.attrHeight || info.renderedHeight;
      ...
      // Parse dimensions from label — supports "1200x1200" and "1:1 (1200x1200)"
      const labelDimMatch = info.label.match(/(\d{2,4})[x×_](\d{2,4})/i);
      if (labelDimMatch) {
