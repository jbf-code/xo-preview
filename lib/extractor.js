  ...
  try {
    ...
    const iframeInfo = await page.evaluate(() => {
      return Array.from(document.querySelectorAll('iframe')).map((el, i) => {
        ...
        // Find dimension label from ancestors
        const findLabel = (container) => {
          if (!container) return null;
          const allText = container.innerText || '';
          const dimMatch = allText.match(/\b(\d{2,4})\s*[x×]\s*(\d{2,4})\b/i);
          if (dimMatch) return `${dimMatch[1]}x${dimMatch[2]}`;
          const labelEl = container.querySelector('[class*="label"], [class*="size"], [class*="format"], [class*="dimension"]');
        ...
        let label = null;
        let ancestor = parent;
        for (let j = 0; j < 5 && ancestor && !label; j++) {
          label = findLabel(ancestor);
    ...
    for (const info of bannerIframes) {
      ...
      const embeddedHtml = html ? await embedFonts(html) : '';
