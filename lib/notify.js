...
// Migration: add monitor_ids column for multi-monitor rules
try { db.exec('ALTER TABLE notification_rules ADD COLUMN monitor_ids TEXT'); } catch (_) {}
...
function addRule({ monitor_ids, recipient_id, on_down = 1, on_recovery = 1, cooldown_min = 15, quiet_start = '22:00', quiet_end = '07:00' }) {
  // monitor_ids: null/undefined = all monitors, array = specific monitors
  const ids = Array.isArray(monitor_ids) && monitor_ids.length > 0 ? JSON.stringify(monitor_ids.map(Number)) : null;
  return db.prepare(
    'INSERT INTO notification_rules (monitor_ids, recipient_id, on_down, on_recovery, cooldown_min, quiet_start, quiet_end) VALUES (?,?,?,?,?,?,?)'
  ).run(ids, recipient_id, on_down, on_recovery, cooldown_min, quiet_start, quiet_end).lastInsertRowid;
}
...
function getGraphToken() {
  ...
  const tenantId = process.env.M365_EMAIL_TENANT_ID;
  const clientId = process.env.M365_EMAIL_CLIENT_ID;
  const clientSecret = process.env.M365_EMAIL_CLIENT_SECRET;
  if (!tenantId || !clientId || !clientSecret) return Promise.reject(new Error('M365 credentials not set'));
...
function sendEmail(to, subject, html) {
  const https = require('https');
  const fromAddr = process.env.STUDIO_EMAIL_FROM || 'page@xo.dk';
  ...
  return getGraphToken().then(token => {
    ...
    return new Promise((resolve, reject) => {
      const req = https.request({
        ...
        path: `/v1.0/users/${fromAddr}/sendMail`,
        method: 'POST',
        headers: {
        ...
        res.on('data', c => d += c);
        res.on('end', () => {
          if (res.statusCode === 202) resolve({ accepted: [to] });
          else reject(new Error(`Graph API ${res.statusCode}: ${d.substring(0, 200)}`));
        });
      });
      req.on('error', reject);
...
async function dispatchNotifications(monitor, event, responseMs) {
  // Fetch all active rules and filter in JS (supports monitor_ids JSON array)
  ...
  const rules = allRules.filter(r => {
    if (r.monitor_ids) {
      // New multi-monitor rule: check if this monitor is in the list
      try { return JSON.parse(r.monitor_ids).map(Number).includes(Number(monitor.id)); }
      catch { return false; }
    }
    // Legacy: monitor_id NULL = all, or specific single ID
    return r.monitor_id === null || r.monitor_id === monitor.id;
  });
