...
function sanitizeBannerHtml(html) {
  ...
  // Scripts and fonts are already inlined by extractor — just rename zuuvi references
  let result = html;
  
  // Step 2: Consistently rename all JS variable/function references
  // These must be done atomically to avoid partial replacements
  const jsReplacements = [
    // Functions and variables
    ['zuuviAdTracking', 'studioAdTracking'],
    ['zuuviTimeline', 'studioTimeline'],
    ['ZuuviTimeline', 'StudioTimeline'],
    ['playZuuviTimeline', 'playStudioTimeline'],
    ['playxo-studioTimeline', 'playStudioTimeline'],
    ['autoplayZuuviTimeline', 'autoplayStudioTimeline'],
    ['autoplayXO StudioTimeline', 'autoplayStudioTimeline'],
    ['xo-studioAdTracking', 'studioAdTracking'],
    ['xo-studioTimeline', 'studioTimeline'],
    ['_zuuvi', '_studio'],
    // Message types
    ['zuuvi-click', 'studio-click'],
    ['zuuvi_click', 'studio_click'],
    // CDN domains (in non-URL contexts like comments)
    ['zuuvicdn.com', 'cdn.studio'],
    ['zuuviapi', 'studioapi'],
  ];
  ...
  // Step 3: Replace remaining zuuvi text in meta/comments/attributes
  result = result
    .replace(/<meta[^>]*zuuvi[^>]*>/gi, '')
    .replace(/<!--[^]*?zuuvi[^]*?-->/gi, '')
    .replace(/(class|id|name|data-[a-z-]+)="([^"]*?)zuuvi([^"]*?)"/gi, '$1="$2studio$3"')
    .replace(/>([^<]*?)Zuuvi([^<]*?)</g, '>$1XO Studio$2<')
    .replace(/>([^<]*?)zuuvi([^<]*?)</gi, '>$1studio$2<');
  
  // Step 4: Clean up any remaining "zuuvi" in non-URL contexts
  // Be careful not to break asset URLs (assets.zuuvi.com, static.zuuvicdn.com)
