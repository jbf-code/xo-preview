  ...
  const bannerItems = banners.map((b, idx) => {
    ...
    return `
      <div class="banner-item" id="banner-${idx}" data-index="${idx}">
        ...
        <div class="banner-wrapper" data-orig-w="${w}" data-orig-h="${h}">
          <div class="banner-scale-container" style="width:${w}px;height:${h}px;transform-origin:top left;">
            ${content}
          </div>
        </div>
      </div>`;
...
<script>
  ...
  function scaleBanners() {
    // Get actual available width from main-content (minus sidebar + padding)
    const mainContent = document.querySelector('.main-content');
    if (!mainContent) return;
    const availableW = mainContent.clientWidth - parseFloat(getComputedStyle(mainContent).paddingLeft) - parseFloat(getComputedStyle(mainContent).paddingRight);
    ...
    document.querySelectorAll('.banner-wrapper').forEach(wrapper => {
      ...
      const scale = Math.min(1, availableW / origW);
      const scaleContainer = wrapper.querySelector('.banner-scale-container');
      if (scaleContainer) {
        scaleContainer.style.transform = 'scale(' + scale + ')';
        scaleContainer.style.transformOrigin = 'top left';
      }
      wrapper.style.width = Math.round(origW * scale) + 'px';
      wrapper.style.height = Math.round(origH * scale) + 'px';
    });
  }
  // Run after layout is ready
  requestAnimationFrame(() => { scaleBanners(); });
