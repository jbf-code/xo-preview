...
<script>
  ...
  function scaleBanners() {
    document.querySelectorAll('.banner-wrapper').forEach(wrapper => {
      ...
      // Get available width from the banner-item parent
      const containerW = wrapper.closest('.banner-item')?.clientWidth || wrapper.parentElement.clientWidth;
      if (!containerW || containerW <= 0) return;
      const scale = Math.min(1, containerW / origW);
  ...
  // Re-scale after fonts/iframes load
