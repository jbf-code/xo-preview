...
function generatePreviewHtml({ id, campaignName, clientName, banners, zuuviUrl, theme }) {
  ...
  const bannerItems = banners.map((b, idx) => {
    ...
    return `
      <div class="banner-item" id="banner-${idx}" data-index="${idx}">
        ...
        <div class="banner-wrapper" data-orig-w="${w}" data-orig-h="${h}" style="width:${displayW}px;height:${displayH}px;position:relative;">
          <div class="banner-scale-container" style="width:${w}px;height:${h}px;transform-origin:top left;transform:${scaleTransform};">
            ${content}
          </div>
          <button class="btn-replay-overlay" title="Replay" onclick="replayBanner(${idx})">↺ Replay</button>
        </div>
      </div>`;
  }).join('\n');
  ...
  const headerHtml = sharedHeader({
    ...
    showAuth: true,
    stats: statsHtml,
    logoSrc: theme && theme.logo_base64 ? theme.logo_base64 : null,
  });

  // Theme CSS variable overrides
  // For xo-default: only override accent + header (surface/border come from :root defaults)
  // For custom themes: derive surface/border from bg_color
  let themeVars;
  if (!theme || theme.id === 'xo-default') {
    themeVars = theme
      ? `--header-bg: ${theme.header_color}; --accent: ${theme.accent_color}; --accent-2: ${theme.accent_color}; --accent-dim: color-mix(in srgb, ${theme.accent_color} 18%, transparent);`
      : '--header-bg: var(--surface);';
  } else {
    themeVars = `
    --accent: ${theme.accent_color};
    --accent-2: ${theme.accent_color};
    --accent-dim: color-mix(in srgb, ${theme.accent_color} 18%, transparent);
    --bg: ${theme.bg_color};
    --surface: color-mix(in oklch, ${theme.bg_color} 70%, white 30%);
    --surface2: color-mix(in oklch, ${theme.bg_color} 55%, white 45%);
    --surface3: color-mix(in oklch, ${theme.bg_color} 40%, white 60%);
    --border: color-mix(in oklch, ${theme.bg_color} 45%, white 55%);
    --border2: color-mix(in oklch, ${theme.bg_color} 35%, white 65%);
    --text: #e8e8e8;
    --text-2: #aaa;
    --text-3: #666;
...
<head>
  ...
  <style>
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .topbar { position: fixed; top: 0; left: 0; right: 0; z-index: 1000; height: 90px; }
    ...
    .btn-replay-overlay { position: absolute; bottom: 8px; right: 8px; font-size: 11px; font-weight: 600; padding: 4px 10px; border-radius: var(--radius); border: 1px solid rgba(255,255,255,0.2); background: rgba(0,0,0,0.55); color: #fff; cursor: pointer; backdrop-filter: blur(4px); transition: background 0.15s; z-index: 10; }
    .btn-replay-overlay:hover { background: rgba(0,0,0,0.8); }
...
<script>
  ...
  function playAllBanners() {
    ...
    if (btn.classList.contains('active')) {
      btn.textContent = '■';
      // Replay all banners by resetting their srcdoc
      document.querySelectorAll('.banner-iframe').forEach(iframe => {
        const src = iframe.getAttribute('srcdoc');
        iframe.removeAttribute('srcdoc');
        setTimeout(() => iframe.setAttribute('srcdoc', src), 50);
      });
    } else {
      btn.textContent = '▶';
    }
