    ...
    return `
      <div class="banner-item" id="banner-${idx}" data-index="${idx}">
        ...
        <div class="banner-wrapper" data-orig-w="${w}" data-orig-h="${h}" style="width:${displayW}px;height:${displayH}px;">
          <button class="btn-replay" title="Replay" onclick="replayBanner(${idx})">↺</button>
...
<head>
  ...
  <style>
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .btn-replay { position: absolute; top: 8px; right: 8px; z-index: 10; width: 32px; height: 32px; border-radius: 50%; border: 1px solid rgba(255,255,255,0.2); background: rgba(0,0,0,0.6); color: #fff; font-size: 16px; cursor: pointer; display: flex; align-items: center; justify-content: center; opacity: 0; transition: opacity 0.2s; }
    .banner-wrapper:hover .btn-replay { opacity: 1; }
    .btn-replay:hover { background: rgba(0,0,0,0.85); }
