...
function sanitizeBannerHtml(html) {
  if (!html) return html;
  // Remove zuuvi references from visible text, meta tags, comments — but preserve asset URLs
  return html
    // Remove meta tags with zuuvi
    .replace(/<meta[^>]*zuuvi[^>]*>/gi, '')
    // Remove HTML comments mentioning zuuvi
    .replace(/<!--[^]*?zuuvi[^]*?-->/gi, '')
    // Replace zuuvi in class/id attributes (not in src/href URLs)
    .replace(/(class|id|name|data-[a-z-]+)="([^"]*?)zuuvi([^"]*?)"/gi, '$1="$2xo-studio$3"')
    // Replace visible text "Zuuvi" (not inside URLs)
    .replace(/>([^<]*?)Zuuvi([^<]*?)</g, '>$1XO Studio$2<')
    .replace(/>([^<]*?)zuuvi([^<]*?)</gi, '>$1xo-studio$2<');
}
