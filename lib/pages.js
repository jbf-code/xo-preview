    ...
    <tr id="edit-${safeEmail}" style="display:none;background:var(--surface2);">
      <td colspan="4" style="padding:12px 16px;">
        <div style="display:grid;grid-template-columns:1fr 1fr 1fr auto;gap:10px;align-items:end;">
          ${u.fromDb ? `<div><label class="label">Navn</label><input class="input" id="edit-name-${safeEmail}" value="${safeName}"></div>` : '<div></div>'}
          ...
          <div>
            <button class="btn btn-primary" onclick="saveEdit('${safeEmailJs}', ${u.fromDb ? 'true' : 'false'})">Gem</button>
            <button class="btn btn-secondary" style="margin-left:4px;" onclick="toggleEdit('${safeEmailJs}')">Annuller</button>
          </div>
        </div>
        <div id="edit-err-${safeEmail}" style="display:none;color:#ef4444;font-size:12px;margin-top:8px;"></div>
      </td>
    </tr>`;
  }).join('');
...
function saveEdit(email, isDbUser) {
  ...
  var body = { telegram_chat_id: tgEl ? tgEl.value.trim() || null : undefined };
  if (isDbUser) {
  ...
  fetch('/settings/users/' + encodeURIComponent(email), {
    ...
    if (d.ok) { location.reload(); }
    else { if (errEl) { errEl.textContent = d.error || 'Fejl'; errEl.style.display = 'block'; } }
  });
}
