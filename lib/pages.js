...
function monitor(monitors, ctx = {}) {
  const uptime  = (v) => v === null ? '\u2014' : v + '%';
  const ms      = (v) => v === null ? '\u2014' : v + 'ms';
  const lastSeen = (m) => {
    if (!m.last) return 'Never';
    const diff = Math.floor((Date.now() - new Date(m.last.checked_at + 'Z').getTime()) / 1000);
  ...
  const overallClr  = monitors.length === 0 ? 'var(--text-3)' : anyDown ? '#e74c3c' : '#2ecc71';
  const overallIcon = monitors.length === 0 ? '\u26aa' : anyDown ? '\ud83d\udd34' : '\ud83d\udfe2';
  const overallTxt  = monitors.length === 0 ? 'No monitors' : anyDown ? 'Some services are down' : 'All systems operational';

  const cards = monitors.map(m => {
    const isUp      = m.last && m.last.status === 'up';
    const isDown    = m.last && m.last.status === 'down';
    const dotClr    = !m.last ? 'var(--text-3)' : isUp ? '#2ecc71' : '#e74c3c';
    const borderClr = !m.last ? 'var(--border)' : isUp ? '#2ecc7133' : '#e74c3c44';
    const badgeSt   = !m.last
      ? 'color:var(--text-3);background:var(--surface2);border:1px solid var(--border2);'
      : isUp ? 'color:#2ecc71;background:#2ecc7118;border:1px solid #2ecc7133;'
             : 'color:#e74c3c;background:#e74c3c18;border:1px solid #e74c3c44;';
    const badgeTxt  = !m.last ? 'No data' : isUp ? 'UP' : 'DOWN';
    const pulse     = isDown ? 'animation:pulse-red 1.5s infinite;' : '';
    const errHtml   = m.last && m.last.error
      ? '<div style="font-size:11px;color:#e74c3c;background:#e74c3c18;border:1px solid #e74c3c33;border-radius:4px;padding:4px 8px;margin-bottom:8px;">' + escHtml(m.last.error) + '</div>'
      : '';

    return '<div class="card" style="padding:16px 20px;border-left:3px solid ' + borderClr + ';">'
      + '<div style="display:flex;align-items:center;gap:10px;margin-bottom:6px;">'
      + '<span style="width:8px;height:8px;border-radius:50%;background:' + dotClr + ';flex-shrink:0;' + pulse + '"></span>'
      + '<span style="font-size:14px;font-weight:700;color:var(--text);flex:1;">' + escHtml(m.name) + '</span>'
      + '<span style="font-size:11px;font-weight:700;border-radius:20px;padding:2px 10px;' + badgeSt + '">' + badgeTxt + '</span>'
      + '</div>'
      + '<div style="font-size:11px;color:var(--text-3);margin-bottom:10px;font-family:var(--mono);">' + escHtml(m.target) + '</div>'
      + '<div style="display:grid;grid-template-columns:repeat(4,1fr);gap:6px;margin-bottom:10px;">'
      + stat('Uptime 24h', uptime(m.uptime24h))
      + stat('Uptime 7d',  uptime(m.uptime7d))
      + stat('Avg resp.',  ms(m.avgMs))
      + stat('Checked',    lastSeen(m))
      + '</div>'
      + errHtml
      + '<div style="display:flex;gap:8px;">'
      + '<button class="btn btn-secondary btn-sm" onclick="checkNow(' + m.id + ')">Check now</button>'
      + '<button class="btn btn-danger btn-sm" onclick="delMonitor(' + m.id + ',\'' + escHtml(m.name).replace(/'/g, '') + '\')">Remove</button>'
      + '</div></div>';
  }).join('');
  ...
  const body = '<style>@keyframes pulse-red{0%,100%{opacity:1}50%{opacity:.3}}</style>'
    + '<div style="max-width:1200px;margin:0 auto;padding:32px 24px;">'
    ...
    + overallIcon + ' <span style="color:' + overallClr + ';">' + overallTxt + '</span>'
    + ' &nbsp;&bull;&nbsp; ' + monitors.length + ' monitors</p></div>'
    + '</div>'

    + '<div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(300px,1fr));gap:16px;margin-bottom:32px;">'
    + (cards || '<div class="card" style="grid-column:1/-1;text-align:center;padding:48px;color:var(--text-3);">No monitors yet</div>')
    + '</div>'
    ...
    + '<div style="display:grid;grid-template-columns:2fr 1fr 3fr 1fr auto;gap:12px;align-items:end;">'
    ...
    + '<div><button class="btn btn-primary" onclick="addMonitor()">+ Add</button></div>'
    + '</div></div>'
    ...
    + '<div class="card">'
    + '<h3 style="font-size:13px;font-weight:700;color:var(--text-2);text-transform:uppercase;letter-spacing:.05em;margin-bottom:12px;">Recipients</h3>'
    + '<div id="recipientList" style="margin-bottom:16px;"><span style="color:var(--text-3);font-size:13px;">Loading...</span></div>'
    + '<div style="border-top:1px solid var(--border);padding-top:16px;display:grid;grid-template-columns:1fr 1fr;gap:10px;">'
    + '<div><label class="label">Name</label><input class="input" id="rName" placeholder="JB"></div>'
    + '<div><label class="label">Telegram chat ID</label><input class="input" id="rTelegram" placeholder="8326264778"></div>'
    + '<div style="grid-column:1/-1;"><label class="label">Email (optional)</label><input class="input" type="email" id="rEmail" placeholder="you@example.com"></div>'
    + '<div style="grid-column:1/-1;"><button class="btn btn-primary btn-sm" onclick="addRecipient()">Add recipient</button></div>'
    + '</div></div>'
    ...
    + '<div style="border-top:1px solid var(--border);padding-top:16px;display:grid;grid-template-columns:1fr 1fr;gap:10px;">'
    + '<div><label class="label">Recipient</label><select class="input" id="rulRecipient"><option value="">Select...</option></select></div>'
    + '<div><label class="label">Monitor (blank = all)</label><select class="input" id="rulMonitor"><option value="">All monitors</option></select></div>'
    ...
    + '<script src="/monitor-notif.js"><\/script>'
    + '</div>';
