    ...
    <tr class="${statusClass}">
      ...
      <td style="padding:12px 16px;">
        <div style="display:flex;align-items:center;gap:6px;">
          ...
          <form method="POST" action="/delete/${escHtml(p.id)}" onsubmit="return confirm('⚠️ Slet preview: ${escHtml(p.name).replace(/'/g, "\\'")}\'\n\nDette kan ikke fortrydes!')" style="margin:0;">
    ...
    <tr>
      ...
      <td style="padding:12px 16px;">
        <div style="display:flex;align-items:center;gap:6px;">
          ...
          <form method="POST" action="/hosting/delete/${escHtml(c.id)}" onsubmit="return confirm('⚠️ Slet hosted campaign: ${escHtml(c.name).replace(/'/g, "\\'")}\'\n\nAlle filer på CDN slettes også!\nDette kan ikke fortrydes!')" style="margin:0;">
            <button type="submit" class="btn btn-danger btn-sm">🗑</button>
          </form>
        </div>
      </td>
