...
function settings({ users, previewCount, hostingCount, passwordChanged, passwordError, themes }, ctx) {
  const userRows = (users || []).map(u => `
    <tr>
      <td style="padding:10px 16px;font-weight:500;">${escHtml(u.name)}
        ${u.fromDb ? '<span style="font-size:10px;color:var(--text-3);margin-left:4px;">(DB)</span>' : ''}
      </td>
      <td style="padding:10px 16px;color:var(--text-2);font-size:13px;">${escHtml(u.email)}</td>
      <td style="padding:8px 16px;">
        <div style="display:flex;gap:8px;align-items:center;">
          <input class="input" style="padding:6px 10px;font-size:13px;width:160px;" type="text"
            id="tg-${escHtml(u.email)}" value="${escHtml(u.telegram_chat_id || '')}" placeholder="Chat ID">
          <button class="btn btn-secondary btn-sm" onclick="saveTelegram('${escHtml(u.email)}')">Gem</button>
          ${u.telegram_chat_id ? '<span class="tg-check" style="font-size:11px;color:#2ecc71;">&#10003;</span>' : ''}
        </div>
      </td>
      <td style="padding:8px 16px;">
        ${u.fromDb ? `<button class="btn btn-danger btn-sm" onclick="deleteUser('${escHtml(u.email)}', '${escHtml(u.name)}')">Slet</button>` : '<span style="font-size:11px;color:var(--text-3);">Env</span>'}
      </td>
    </tr>`).join('');
...
<div style="max-width:1200px;margin:0 auto;padding:32px 24px;">
  ...
  <div class="card" style="padding:24px;margin-bottom:24px;" id="addUserCard">
    ...
    <div style="display:grid;grid-template-columns:1fr 1fr 1fr auto;gap:12px;align-items:end;">
      ...
      <div><button class="btn btn-primary" onclick="createUser()">Opret</button></div>
    </div>
  </div>
...
function monitor(ctx = {}) {
  ...
  const body = '<style>@keyframes pulse-red{0%,100%{opacity:1}50%{opacity:.3}} @keyframes spin{to{transform:rotate(360deg)}}</style>'
    ...
    + '<div><label class="label">Monitor (blank = alle)</label><select class="input" id="rulMonitor"><option value="">Alle monitors</option></select></div>'
