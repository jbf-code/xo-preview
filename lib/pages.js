...
function hosting({ campaigns }, ctx) {
  const rows = campaigns.map(c => {
    ...
    const impressions = c.cdn_requests || c.total_impressions || c.views || 0;
...
<div style="max-width:1200px;margin:0 auto;padding:32px 24px;">
  <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:24px;">
    <div>
      ...
      <p style="font-size:13px;color:var(--text-3);margin-top:4px;">${campaigns.length} campaign${campaigns.length !== 1 ? 's' : ''} · ${campaigns.reduce((s,c) => s + (c.format_count_live || c.format_count || 0), 0)} formater · ${campaigns.reduce((s,c) => s + (c.cdn_requests || 0), 0).toLocaleString()} impressions · ${campaigns.reduce((s,c) => s + (c.cdn_mb_served || 0), 0).toFixed(1)} MB served</p>
    </div>
    <div style="display:flex;gap:8px;">
      <form method="POST" action="/hosting/backfill-clicks" style="margin:0;"><button type="submit" class="btn btn-secondary">🔗 Scan Click URLs</button></form>
    ...
    <div class="card" style="padding:20px;margin-bottom:16px;">
      <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:16px;">
        <div style="display:flex;align-items:center;gap:10px;">
          ...
          <span style="font-size:11px;color:var(--text-3);margin-left:8px;">📁 ${f.file_count || 0} filer · 👁 ${f.cdn_requests || 0} requests · 📦 ${f.cdn_mb_served ? f.cdn_mb_served.toFixed(2) + ' MB' : '—'} served</span>
          ${f.click_url ? `<span style="font-size:11px;color:var(--text-3);margin-left:8px;">🔗 <a href="${escHtml(f.click_url)}" target="_blank" style="color:var(--accent);">${escHtml(f.click_url)}</a></span>` : ''}
        </div>
...
<div style="max-width:1200px;margin:0 auto;padding:32px 24px;">
  <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:24px;">
    <div>
      ...
      <p style="font-size:13px;color:var(--text-3);margin-top:4px;">Oprettet af ${escHtml(campaign.created_by || 'unknown')} · ${new Date(campaign.created_at).toLocaleString('da-DK',{dateStyle:'short',timeStyle:'short'})}</p>
    </div>
    <a href="/hosting" class="btn btn-secondary">← Tilbage</a>
  </div>
  ${statusHtml}
