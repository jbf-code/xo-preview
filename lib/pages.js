...
function hosting({ campaigns }, ctx) {
  const rows = campaigns.map(c => `
    <tr>
      <td><a href="/hosting/${escHtml(c.id)}" style="color:var(--accent);font-weight:600;">${escHtml(c.name)}</a></td>
      <td><span class="status-${c.status}">${c.status}</span></td>
      <td>${escHtml(c.created_by || '')}</td>
      <td>${new Date(c.created_at).toLocaleDateString('da-DK')}</td>
      <td>
        <form method="POST" action="/hosting/delete/${escHtml(c.id)}" style="display:inline;">
          <button type="submit" class="btn-sm btn-danger" onclick="return confirm('Slet denne hosted campaign?')">Slet</button>
        </form>
      </td>
    </tr>`).join('');
  ...
  const body = `
    <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:24px;">
      ...
      <a href="/hosting/new" class="btn-primary">+ Upload Banner Package</a>
    </div>
    ${campaigns.length === 0 ? '<p style="color:var(--text-3);">Ingen hosted campaigns endnu. Upload en Zuuvi export ZIP for at starte.</p>' : `
    <table class="data-table">
      <thead><tr><th>Campaign</th><th>Status</th><th>Oprettet af</th><th>Dato</th><th></th></tr></thead>
      <tbody>${rows}</tbody>
    </table>`}`;
...
function hostingNew({ error }, ctx) {
  const body = `
    <h1 style="font-size:22px;font-weight:700;margin-bottom:24px;">Upload Banner Package</h1>
    ${error ? `<div class="error-msg">${escHtml(error)}</div>` : ''}
    <form method="POST" action="/hosting/upload" enctype="multipart/form-data" style="max-width:500px;">
      <div style="margin-bottom:16px;">
        <label class="label" for="name">Campaign Name</label>
        <input class="input" type="text" id="name" name="name" placeholder="e.g. Ørsted TruePower Spring 2026" required>
      </div>
      <div style="margin-bottom:16px;">
        <label class="label" for="zipfile">Banner ZIP</label>
        <p style="font-size:12px;color:var(--text-3);margin:4px 0 8px;">Upload en Zuuvi "Google Ad Manager Display" export (ZIP med format-ZIPs indeni) eller en enkelt banner-pakke.</p>
        <input class="input" type="file" id="zipfile" name="zipfile" accept=".zip" required>
      </div>
      <button type="submit" class="btn-primary">Upload & Host</button>
    </form>`;
...
function hostingDetail({ campaign, formats }, ctx) {
  ...
  let statusHtml = '';
  if (isProcessing) statusHtml = '<div style="padding:16px;background:var(--accent-dim);border:1px solid var(--accent);border-radius:var(--radius);margin-bottom:24px;"><strong>⏳ Processing...</strong> Upload og behandling er i gang. Genindlæs om et øjeblik.</div>';
  ...
  const formatRows = formats.map(f => `
    <div style="border:1px solid var(--border);border-radius:var(--radius);padding:16px;margin-bottom:16px;">
      <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:12px;">
        <div>
          <span class="format-chip">${escHtml(f.format_name)}</span>
          <span style="font-size:12px;color:var(--text-3);margin-left:8px;">${f.width}×${f.height}</span>
        </div>
        <a href="${escHtml(f.cdn_url)}" target="_blank" style="font-size:12px;color:var(--accent);">Preview ↗</a>
      </div>
      <div style="margin-bottom:8px;">
        <label style="font-size:11px;font-weight:600;color:var(--text-3);text-transform:uppercase;">Google Ad Manager Tag</label>
        <div style="position:relative;">
          <textarea readonly style="width:100%;height:60px;font-family:var(--mono);font-size:11px;background:var(--surface2);border:1px solid var(--border);border-radius:4px;padding:8px;resize:none;color:var(--text);">${escHtml(f.tag_html)}</textarea>
          <button onclick="this.previousElementSibling.select();document.execCommand('copy');this.textContent='✓'" style="position:absolute;top:4px;right:4px;font-size:10px;padding:2px 8px;border:1px solid var(--border);border-radius:3px;background:var(--surface);cursor:pointer;color:var(--text-2);">Copy</button>
        </div>
      </div>
      <div>
        <label style="font-size:11px;font-weight:600;color:var(--text-3);text-transform:uppercase;">Direct URL</label>
        <input readonly value="${escHtml(f.cdn_url)}" style="width:100%;font-family:var(--mono);font-size:11px;background:var(--surface2);border:1px solid var(--border);border-radius:4px;padding:6px 8px;color:var(--text);" onclick="this.select()">
      </div>
    </div>`).join('');
  ...
  const body = `
    <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:24px;">
      <div>
        ...
        <p style="font-size:12px;color:var(--text-3);margin-top:4px;">Created by ${escHtml(campaign.created_by || 'unknown')} · ${new Date(campaign.created_at).toLocaleDateString('da-DK')}</p>
      </div>
      <a href="/hosting" style="font-size:13px;color:var(--text-2);text-decoration:none;">← Tilbage</a>
    </div>
    ${statusHtml}
    ${isProcessing ? '<meta http-equiv="refresh" content="3">' : ''}
    ${isReady ? `<p style="margin-bottom:16px;font-size:14px;"><strong>${formats.length}</strong> format(er) hosted</p>` : ''}
    ${formatRows}`;
