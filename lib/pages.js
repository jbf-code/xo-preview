    ...
    <tr class="${statusClass}">
      ...
      <td style="padding:12px 16px;">
        <div style="display:flex;align-items:center;gap:6px;">
          ...
          <button class="btn btn-secondary btn-sm edit-toggle-btn" data-id="${escHtml(p.id)}">✏️ Redigér</button>
          <form method="POST" action="/delete/${escHtml(p.id)}" class="delete-form" data-name="${escHtml(p.name)}" data-type="preview" style="margin:0;">
      ...
      <td colspan="5" style="padding:16px 24px;">
        <form method="POST" action="/edit/${escHtml(p.id)}" style="display:flex;flex-direction:column;gap:12px;max-width:600px;">
          ...
          <div style="display:flex;gap:8px;">
            ...
            <button type="button" class="btn btn-secondary btn-sm edit-toggle-btn" data-id="${escHtml(p.id)}">Annullér</button>
          </div>
        </form>
      </td>
    </tr>`;
...
document.querySelectorAll('.edit-toggle-btn').forEach(function(btn) {
  btn.addEventListener('click', function() {
    var row = document.getElementById('edit-' + this.dataset.id);
...
document.querySelectorAll('.delete-form').forEach(function(form) {
  form.addEventListener('submit', function(e) {
    var name = this.dataset.name || 'dette element';
    var type = this.dataset.type;
    var msg = type === 'hosting'
      ? '⚠️ Slet hosted campaign: ' + name + '\n\nAlle filer på CDN slettes også!\nDette kan ikke fortrydes!'
      : '⚠️ Slet preview: ' + name + '\n\nDette kan ikke fortrydes!';
...
// ── Settings page ─────────────────────────────────────────────────────────────
function settings({ users, previewCount, hostingCount, passwordChanged, passwordError }, ctx) {
    ...
    <tr>
      ...
      <td style="padding:12px 16px;">
        <div style="display:flex;align-items:center;gap:6px;">
          ...
          <form method="POST" action="/hosting/delete/${escHtml(c.id)}" class="delete-form" data-name="${escHtml(c.name)}" data-type="hosting" style="margin:0;">
            <button type="submit" class="btn btn-danger btn-sm">🗑</button>
          </form>
        </div>
      </td>
...
document.querySelectorAll('.delete-form').forEach(function(form) {
  form.addEventListener('submit', function(e) {
    var name = this.dataset.name || 'dette element';
    var msg = '⚠️ Slet hosted campaign: ' + name + '\n\nAlle filer på CDN slettes også!\nDette kan ikke fortrydes!';
    if (!confirm(msg)) e.preventDefault();
  });
});
</script>`;
...
module.exports = { login, dashboard, new: newPreview, generating, error, settings, hosting, 'hosting-new': hostingNew, 'hosting-detail': hostingDetail, 'hosting-replace': hostingReplace, 'hosting-replace-confirm': hostingReplaceConfirm, sharedHeader, getSharedHeaderCSS, XO_LOGO_BASE64, escHtml };
