...
function hosting({ campaigns }, ctx) {
  const rows = campaigns.map(c => {
    ...
    const sizeMB = c.total_size_bytes ? (c.total_size_bytes / 1024 / 1024).toFixed(1) + ' MB' : '—';
    return `
    <tr>
      <td style="padding:12px 16px;white-space:nowrap;">
      ...
      <td style="padding:12px 16px;font-size:12px;font-weight:500;">${statusText}</td>
      <td style="padding:12px 16px;font-size:12px;color:var(--text-2);font-family:var(--mono);">${c.format_count || 0} formater</td>
      <td style="padding:12px 16px;font-size:12px;color:var(--text-2);font-family:var(--mono);">${sizeMB}</td>
      <td style="padding:12px 16px;font-size:12px;text-align:center;font-weight:600;">${c.views || 0}</td>
      <td style="padding:12px 16px;">
        <div style="display:flex;align-items:center;gap:6px;">
  ...
  const empty = campaigns.length === 0
    ? '<tr><td colspan="6" style="text-align:center;padding:60px 0;color:var(--text-3);">Ingen hosted campaigns endnu</td></tr>'
    : '';
...
<div style="max-width:1200px;margin:0 auto;padding:32px 24px;">
  <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:24px;">
    <div>
      ...
      <p style="font-size:13px;color:var(--text-3);margin-top:4px;">${campaigns.length} campaign${campaigns.length !== 1 ? 's' : ''} hosted</p>
    </div>
    ...
    <div class="card" style="padding:20px;margin-bottom:16px;">
      <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:16px;">
        <div style="display:flex;align-items:center;gap:10px;">
          ...
          <span style="font-size:11px;color:var(--text-3);margin-left:8px;">📁 ${f.file_count || 0} filer · ${f.size_bytes ? (f.size_bytes / 1024).toFixed(0) + ' KB' : '—'} · 👁 ${f.views || 0} impressions</span>
        </div>
