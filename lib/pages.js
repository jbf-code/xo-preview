...
function monitor(monitors, ctx = {}) {
  const statusBadge = (m) => {
    if (!m.last) return '<span class="mon-badge mon-unknown">No data</span>';
    return m.last.status === 'up'
      ? '<span class="mon-badge mon-up">UP</span>'
      : '<span class="mon-badge mon-down">DOWN</span>';
  };

  const dot = (m) => {
    if (!m.last) return '<span class="mon-dot mon-dot-unknown"></span>';
    return m.last.status === 'up'
      ? '<span class="mon-dot mon-dot-up"></span>'
      : '<span class="mon-dot mon-dot-down"></span>';
  };

  const uptime = (val) => val === null ? '—' : `${val}%`;
  const ms = (val) => val === null ? '—' : `${val} ms`;
  const lastSeen = (m) => {
    if (!m.last) return 'Never checked';
    const d = new Date(m.last.checked_at + 'Z');
  ...
  const allUp = monitors.length > 0 && monitors.every(m => !m.last || m.last.status === 'up');
  const anyDown = monitors.some(m => m.last && m.last.status === 'down');
  const overallStatus = monitors.length === 0
    ? '<span class="mon-overall mon-overall-unknown">⚪ No monitors configured</span>'
    : anyDown
      ? '<span class="mon-overall mon-overall-down">🔴 Some services are down</span>'
      : '<span class="mon-overall mon-overall-up">🟢 All systems operational</span>';

  const cards = monitors.map(m => `
    <div class="mon-card ${m.last ? (m.last.status === 'up' ? 'mon-card-up' : 'mon-card-down') : 'mon-card-unknown'}">
      <div class="mon-card-header">
        ${dot(m)}
        <div class="mon-card-name">${escHtml(m.name)}</div>
        ${statusBadge(m)}
      </div>
      <div class="mon-card-target">${escHtml(m.target)}</div>
      <div class="mon-card-stats">
        <div class="mon-stat"><span class="mon-stat-label">Uptime 24h</span><span class="mon-stat-val">${uptime(m.uptime24h)}</span></div>
        <div class="mon-stat"><span class="mon-stat-label">Uptime 7d</span><span class="mon-stat-val">${uptime(m.uptime7d)}</span></div>
        <div class="mon-stat"><span class="mon-stat-label">Avg resp.</span><span class="mon-stat-val">${ms(m.avgMs)}</span></div>
        <div class="mon-stat"><span class="mon-stat-label">Checked</span><span class="mon-stat-val">${lastSeen(m)}</span></div>
      </div>
      ${m.last && m.last.status_code ? `<div class="mon-card-code">HTTP ${m.last.status_code}</div>` : ''}
      ${m.last && m.last.error ? `<div class="mon-card-error">${escHtml(m.last.error)}</div>` : ''}
      <div class="mon-card-actions">
        <button class="mon-btn-check" onclick="checkNow(${m.id})">Check now</button>
        <button class="mon-btn-delete" onclick="deleteMonitor(${m.id}, '${escHtml(m.name).replace(/'/g, '')}')">Remove</button>
      </div>
    </div>
  `).join('');
  ...
  const body = `
<style>
.mon-header { display:flex; align-items:center; justify-content:space-between; margin-bottom:24px; flex-wrap:wrap; gap:12px; }
.mon-overall { font-size:1.1rem; font-weight:600; }
.mon-overall-up { color:#16a34a; }
.mon-overall-down { color:#dc2626; }
.mon-overall-unknown { color:#6b7280; }
.mon-grid { display:grid; grid-template-columns:repeat(auto-fill, minmax(280px, 1fr)); gap:16px; margin-bottom:32px; }
.mon-card { background:#fff; border:1px solid #e5e7eb; border-radius:10px; padding:16px; transition:box-shadow .15s; }
.mon-card:hover { box-shadow:0 2px 10px rgba(0,0,0,.07); }
.mon-card-up { border-left:4px solid #22c55e; }
.mon-card-down { border-left:4px solid #ef4444; }
.mon-card-unknown { border-left:4px solid #d1d5db; }
.mon-card-header { display:flex; align-items:center; gap:8px; margin-bottom:6px; }
.mon-dot { width:10px; height:10px; border-radius:50%; flex-shrink:0; }
.mon-dot-up { background:#22c55e; box-shadow:0 0 6px #22c55e88; }
.mon-dot-down { background:#ef4444; box-shadow:0 0 6px #ef444488; animation:pulse-red 1.5s infinite; }
.mon-dot-unknown { background:#d1d5db; }
@keyframes pulse-red { 0%,100%{opacity:1} 50%{opacity:.4} }
.mon-card-name { font-weight:700; font-size:.95rem; flex:1; color:#111827; }
.mon-badge { font-size:.7rem; font-weight:700; padding:2px 8px; border-radius:20px; letter-spacing:.05em; }
.mon-up { background:#dcfce7; color:#15803d; }
.mon-down { background:#fee2e2; color:#b91c1c; }
.mon-unknown { background:#f3f4f6; color:#6b7280; }
.mon-card-target { font-size:.78rem; color:#9ca3af; margin-bottom:10px; white-space:nowrap; overflow:hidden; text-overflow:ellipsis; }
.mon-card-stats { display:grid; grid-template-columns:1fr 1fr; gap:6px; margin-bottom:10px; }
.mon-stat { display:flex; flex-direction:column; background:#f9fafb; border-radius:6px; padding:6px 8px; }
.mon-stat-label { font-size:.7rem; color:#9ca3af; }
.mon-stat-val { font-size:.85rem; font-weight:600; color:#111827; }
.mon-card-code { font-size:.75rem; color:#6b7280; margin-bottom:4px; }
.mon-card-error { font-size:.75rem; color:#dc2626; background:#fef2f2; border-radius:4px; padding:4px 6px; margin-bottom:6px; }
.mon-card-actions { display:flex; gap:8px; }
.mon-btn-check, .mon-btn-delete { border:none; border-radius:6px; padding:5px 12px; font-size:.78rem; cursor:pointer; font-weight:500; }
.mon-btn-check { background:#f3f4f6; color:#374151; }
.mon-btn-check:hover { background:#e5e7eb; }
.mon-btn-delete { background:#fff0f0; color:#dc2626; }
.mon-btn-delete:hover { background:#fee2e2; }
.mon-add { background:#fff; border:1px solid #e5e7eb; border-radius:10px; padding:20px; max-width:560px; }
.mon-add h3 { margin:0 0 16px; font-size:1rem; }
.mon-form-row { display:grid; grid-template-columns:1fr 1fr; gap:12px; margin-bottom:12px; }
.mon-form-row.full { grid-template-columns:1fr; }
.mon-form-group { display:flex; flex-direction:column; gap:4px; }
.mon-form-group label { font-size:.78rem; font-weight:600; color:#374151; }
.mon-form-group input, .mon-form-group select { border:1px solid #d1d5db; border-radius:6px; padding:7px 10px; font-size:.88rem; }
.mon-form-group input:focus, .mon-form-group select:focus { outline:2px solid #6366f1; border-color:transparent; }
.mon-add-btn { background:#6366f1; color:#fff; border:none; border-radius:6px; padding:8px 20px; font-size:.88rem; font-weight:600; cursor:pointer; }
.mon-add-btn:hover { background:#4f46e5; }
.mon-toast { position:fixed; bottom:24px; right:24px; background:#1f2937; color:#fff; padding:10px 18px; border-radius:8px; font-size:.88rem; z-index:999; opacity:0; transition:opacity .3s; pointer-events:none; }
.mon-toast.show { opacity:1; }
</style>

<div class="mon-header">
  ${overallStatus}
  <span style="font-size:.8rem;color:#9ca3af;">Auto-refreshes every 30s</span>
</div>

<div class="mon-grid" id="monGrid">${cards || '<p style="color:#9ca3af">No monitors yet — add one below.</p>'}</div>
...
<div class="mon-add">
  ...
  <div class="mon-form-row">
    <div class="mon-form-group">
      <label>Name</label>
      <input type="text" id="monName" placeholder="My Service" />
    </div>
    <div class="mon-form-group">
      <label>Type</label>
      <select id="monType">
  ...
  <div class="mon-form-row full">
    <div class="mon-form-group">
      <label>Target</label>
      <input type="text" id="monTarget" placeholder="https://example.com  or  host:port" />
    </div>
  </div>
  <div class="mon-form-row">
    <div class="mon-form-group">
      <label>Interval (seconds)</label>
      <input type="number" id="monInterval" value="60" min="15" max="3600" />
    </div>
  </div>
  <button class="mon-add-btn" onclick="addMonitor()">Add monitor</button>
</div>
...
async function addMonitor() {
  ...
  const interval_sec = parseInt(document.getElementById('monInterval').value) || 60;
  if (!name || !target) { toast('Name and target required', false); return; }
  const r = await fetch('/monitor/api/monitors', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ name, type, target, interval_sec }),
  });
  if (r.ok) { toast('Monitor added'); setTimeout(() => location.reload(), 800); }
  else { toast('Failed to add monitor', false); }
}

async function deleteMonitor(id, name) {
  if (!confirm('Remove monitor: ' + name + '?')) return;
  const r = await fetch('/monitor/api/monitors/' + id, { method: 'DELETE' });
  if (r.ok) { toast('Removed'); setTimeout(() => location.reload(), 600); }
  else { toast('Failed to remove', false); }
}

async function checkNow(id) {
  toast('Checking…');
  const r = await fetch('/monitor/api/monitors/' + id + '/check', { method: 'POST' });
  if (r.ok) { const d = await r.json(); toast(d.status === 'up' ? '✅ Up (' + d.response_ms + 'ms)' : '🔴 Down: ' + (d.error || d.status_code)); setTimeout(() => location.reload(), 1200); }
  else { toast('Check failed', false); }
}
...
<h2 style="font-size:1.05rem;font-weight:700;color:#111827;margin:0 0 20px;">Notifications</h2>

<div style="display:grid;grid-template-columns:1fr 1fr;gap:24px;margin-bottom:24px;" class="notif-grid">

  <!-- Recipients -->
  <div class="mon-add">
    <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:14px;">
      <h3 style="margin:0;font-size:1rem;">Recipients</h3>
    </div>
    <div id="recipientList" style="margin-bottom:14px;"><em style="color:#9ca3af;font-size:.85rem;">Loading…</em></div>
    <div style="border-top:1px solid #f3f4f6;padding-top:14px;">
      <div class="mon-form-row">
        <div class="mon-form-group">
          <label>Name</label>
          <input type="text" id="rName" placeholder="JB" />
        </div>
        <div class="mon-form-group">
          <label>Telegram chat ID</label>
          <input type="text" id="rTelegram" placeholder="8326264778" />
        </div>
      </div>
      <div class="mon-form-row full">
        <div class="mon-form-group">
      ...
      <button class="mon-add-btn" onclick="addRecipient()">Add recipient</button>
    </div>
  </div>
  ...
  <div class="mon-add">
    <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:14px;">
      <h3 style="margin:0;font-size:1rem;">Alert rules</h3>
    </div>
    <div id="ruleList" style="margin-bottom:14px;"><em style="color:#9ca3af;font-size:.85rem;">Loading…</em></div>
    <div style="border-top:1px solid #f3f4f6;padding-top:14px;">
      <div class="mon-form-row">
        <div class="mon-form-group">
          <label>Recipient</label>
          <select id="rulRecipient"><option value="">Select…</option></select>
        </div>
        <div class="mon-form-group">
          <label>Monitor (blank = all)</label>
          <select id="rulMonitor"><option value="">All monitors</option></select>
        </div>
      </div>
      <div class="mon-form-row">
        <div class="mon-form-group">
          <label>Cooldown (min)</label>
          <input type="number" id="rulCooldown" value="15" min="1" />
        </div>
        <div class="mon-form-group">
          <label>Quiet hours</label>
          <div style="display:flex;gap:6px;align-items:center;">
            <input type="time" id="rulQuietStart" value="22:00" style="flex:1;" />
            <span style="color:#9ca3af;">-</span>
            <input type="time" id="rulQuietEnd" value="07:00" style="flex:1;" />
          </div>
        </div>
      </div>
      <div style="display:flex;gap:16px;margin-bottom:12px;">
        <label style="display:flex;align-items:center;gap:6px;font-size:.85rem;cursor:pointer;">
          <input type="checkbox" id="rulDown" checked /> Alert on DOWN
        </label>
        <label style="display:flex;align-items:center;gap:6px;font-size:.85rem;cursor:pointer;">
          <input type="checkbox" id="rulRecovery" checked /> Alert on recovery
        </label>
      </div>
      <button class="mon-add-btn" onclick="addRule()">Add rule</button>
    </div>
  </div>
...
<!-- Notification log -->
<div class="mon-add" style="max-width:100%;">
  <h3 style="margin:0 0 14px;font-size:1rem;">Notification log</h3>
  <div id="notifLog"><em style="color:#9ca3af;font-size:.85rem;">Loading…</em></div>
</div>

<style>
@media(max-width:640px){.notif-grid{grid-template-columns:1fr!important;}}
.notif-row{display:flex;align-items:center;gap:8px;padding:7px 0;border-bottom:1px solid #f3f4f6;font-size:.82rem;}
.notif-row:last-child{border:none;}
.notif-event-down{color:#dc2626;font-weight:700;}
.notif-event-recovery{color:#16a34a;font-weight:700;}
.notif-err{color:#dc2626;font-size:.75rem;}
.recip-row,.rule-row{display:flex;align-items:center;justify-content:space-between;padding:6px 0;border-bottom:1px solid #f3f4f6;font-size:.84rem;}
.recip-row:last-child,.rule-row:last-child{border:none;}
.btn-sm{border:none;border-radius:5px;padding:3px 10px;font-size:.76rem;cursor:pointer;font-weight:500;}
.btn-del{background:#fff0f0;color:#dc2626;}
.btn-del:hover{background:#fee2e2;}
.btn-test{background:#f0f4ff;color:#4f46e5;}
...
async function loadNotifData() {
  ...
  rSel.innerHTML='<option value="">Select...</option>'+recipRes.map(function(r){return '<option value="'+r.id+'">'+escH(r.name)+'</option>';}).join('');

  document.getElementById('recipientList').innerHTML = recipRes.length
    ? recipRes.map(function(r){
        var tg = r.telegram_chat_id ? '<span style="color:#6b7280;"> &bull; TG: '+r.telegram_chat_id+'</span>' : '';
        var em = r.email ? '<span style="color:#6b7280;"> &bull; '+r.email+'</span>' : '';
        return '<div class="recip-row"><div><strong>'+escH(r.name)+'</strong>'+tg+em+'</div>'
          +'<div style="display:flex;gap:6px;">'
          +'<button class="btn-sm btn-test" onclick="testRecipient('+r.id+')">Test</button>'
          +'<button class="btn-sm btn-del" onclick="delRecipient('+r.id+')">Remove</button></div></div>';
      }).join('')
    : '<em style="color:#9ca3af;">No recipients</em>';

  var mSel=document.getElementById('rulMonitor');
  mSel.innerHTML='<option value="">All monitors</option>'+monRes.map(function(m){return '<option value="'+m.id+'">'+escH(m.name)+'</option>';}).join('');

  var monMap={}; monRes.forEach(function(m){monMap[m.id]=m.name;});
  document.getElementById('ruleList').innerHTML = ruleRes.length
    ? ruleRes.map(function(r){
        var mName = r.monitor_id ? escH(monMap[r.monitor_id]||'?') : 'All';
        var flags = (r.on_down?' &bull; DOWN':'')+(r.on_recovery?' &bull; Recovery':'');
        return '<div class="rule-row"><div><strong>'+escH(r.recipient_name)+'</strong>'
          +'<span style="color:#6b7280;"> &rarr; '+mName+'</span><br>'
          +'<span style="color:#9ca3af;font-size:.76rem;">Cooldown '+r.cooldown_min+'min &bull; Quiet '+r.quiet_start+'&ndash;'+r.quiet_end+flags+'</span></div>'
          +'<button class="btn-sm btn-del" onclick="delRule('+r.id+')">Remove</button></div>';
      }).join('')
    : '<em style="color:#9ca3af;">No rules</em>';

  document.getElementById('notifLog').innerHTML = logRes.length
    ? logRes.map(function(l){
        var t=new Date(l.sent_at+'Z').toLocaleString('da-DK',{timeZone:'Europe/Copenhagen'});
        var ev=l.event==='down'?'<span class="notif-event-down">&#x1F534; DOWN</span>':'<span class="notif-event-recovery">&#x1F7E2; UP</span>';
        var ch=l.channel==='telegram'?'&#x1F4F1;':'&#x1F4E7;';
        var err=l.error?'<span class="notif-err">'+escH(l.error)+'</span>':'';
        return '<div class="notif-row">'+ev+' '+ch+' <strong>'+escH(l.monitor_name||'?')+'</strong> '
          +'<span style="color:#6b7280;flex:1;">&rarr; '+escH(l.message||'')+'</span>'+err
          +'<span style="color:#9ca3af;white-space:nowrap;">'+t+'</span></div>';
      }).join('')
    : '<em style="color:#9ca3af;">No notifications sent yet</em>';
}
...
async function addRecipient() {
  ...
  const tg = document.getElementById('rTelegram').value.trim();
  const email = document.getElementById('rEmail').value.trim();
  if (!name) { toast('Name required', false); return; }
  if (!tg && !email) { toast('Telegram or email required', false); return; }
  const r = await fetch('/monitor/api/recipients', { method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({name,telegram_chat_id:tg||null,email:email||null}) });
...
async function delRecipient(id) {
  ...
  await fetch('/monitor/api/recipients/'+id, {method:'DELETE'});
  toast('Removed'); loadNotifData();
}

async function testRecipient(id) {
  toast('Sending test…');
  const r = await fetch('/monitor/api/recipients/'+id+'/test', {method:'POST'});
  const d = await r.json();
  const ok = d.results && d.results.every(x=>x.ok);
  toast(ok ? '✅ Test sent!' : '⚠️ Some channels failed — check log', ok);
  setTimeout(loadNotifData, 1500);
}

async function addRule() {
  const recipient_id = parseInt(document.getElementById('rulRecipient').value);
  const monitor_id = document.getElementById('rulMonitor').value || null;
  const cooldown_min = parseInt(document.getElementById('rulCooldown').value) || 15;
  const quiet_start = document.getElementById('rulQuietStart').value;
  const quiet_end = document.getElementById('rulQuietEnd').value;
  const on_down = document.getElementById('rulDown').checked ? 1 : 0;
  const on_recovery = document.getElementById('rulRecovery').checked ? 1 : 0;
  if (!recipient_id) { toast('Select a recipient', false); return; }
  const r = await fetch('/monitor/api/rules', { method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({recipient_id, monitor_id, cooldown_min, quiet_start, quiet_end, on_down, on_recovery}) });
...
async function delRule(id) {
  ...
  await fetch('/monitor/api/rules/'+id, {method:'DELETE'});
  toast('Removed'); loadNotifData();
}
