...
// Fix: TCP port 22 is firewalled from Railway — switch Hetzner servers to HTTP
db.prepare(`UPDATE monitors SET type='http', target='http://116.202.112.183'
  WHERE target IN ('116.202.112.183:22', 'http://116.202.112.183')`).run();
db.prepare(`UPDATE monitors SET name='Hetzner — Doblify Prod'
  WHERE target='http://116.202.112.183'`).run();

db.prepare(`UPDATE monitors SET type='http', target='http://65.109.87.116'
  WHERE target IN ('65.109.87.116:22', 'http://65.109.87.116')`).run();
db.prepare(`UPDATE monitors SET name='Hetzner — Doblify Stage'
  WHERE target='http://65.109.87.116'`).run();

// Fix: rename Page server + set pre-stage as secondary
db.prepare(`UPDATE monitors SET name='Linux — Kontor (Page)', secondary_type='http', secondary_target='http://65.108.237.36:8099'
  WHERE target='65.108.237.36:22'`).run();
...
const toAdd = [
  { name: 'studio.xo.dk',             type: 'http', target: 'https://studio.xo.dk',      interval_sec: 60  },
  { name: 'xo.dk',                    type: 'http', target: 'https://xo.dk',             interval_sec: 60  },
  { name: 'Linux — Kontor (Page)',     type: 'tcp',  target: '65.108.237.36:22',          interval_sec: 120, secondary_type: 'http', secondary_target: 'http://65.108.237.36:8099' },
  { name: 'Hetzner — Doblify Prod',   type: 'http', target: 'http://116.202.112.183',    interval_sec: 120 },
  { name: 'Hetzner — Doblify Stage',  type: 'http', target: 'http://65.109.87.116',      interval_sec: 120 },
  { name: 'Doblify — Pre-stage',      type: 'http', target: 'http://65.108.237.36:8099', interval_sec: 60  },
];
const insertMissing = db.prepare('INSERT INTO monitors (name, type, target, secondary_type, secondary_target, interval_sec) VALUES (?, ?, ?, ?, ?, ?)');
for (const m of toAdd) {
  if (!existingTargets.has(m.target)) {
    insertMissing.run(m.name, m.type, m.target, m.secondary_type || null, m.secondary_target || null, m.interval_sec);
  }
}
...
function addMonitor({ name, type = 'http', target, secondary_type, secondary_target, interval_sec = 60 }) {
  const r = db.prepare(
    'INSERT INTO monitors (name, type, target, secondary_type, secondary_target, interval_sec) VALUES (?, ?, ?, ?, ?, ?)'
  ).run(name, type, target, secondary_type || null, secondary_target || null, interval_sec);
...
async function runCheck(monitor) {
  ...
  const statusChanged = !prev || prev.status !== overallStatus;
  if (statusChanged) {
